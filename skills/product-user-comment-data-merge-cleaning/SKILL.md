...
- Enforce the canonical bundled cleaner configuration, explicit standardized comment-column/platform selection, exact-path overwrite confirmation, and irreversible default audit-log cleanup.
...
10. Run `scripts/clean_excel_comments.py` against the post-filter `评论内容` column, or the standardized column when no platform-specific filter applies. Every CLI invocation must explicitly pass `--target-header 评论内容` and the confirmed `--platform`. It accepts only the bundled canonical `config/comment-cleaner.json`; there is no active platform-specific cleaner exception in the canonical configuration. Do not create external or temporary cleaner configurations. Do not perform any AI review or rewriting of table values.
11. Verify the cleaned `.xlsx` and `.csv`, then run `scripts/cleanup_intermediate_outputs.py` with explicit intermediate paths, protected original/final paths, and exactly two declared `--final-output` paths: the existing final `.xlsx` and `.csv`. Cleanup must stop before deleting any intermediate when either final file is absent or unprotected.
