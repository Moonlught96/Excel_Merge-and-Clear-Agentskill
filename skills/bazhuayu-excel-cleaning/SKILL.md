...
description: Use when handling Bazhuayu/Octoparse Excel or CSV comment exports, including deterministic file merging, header standardization, comment cleaning, workflow validation, or rule-preserving extensions.
---
...
- Pass every original input path and both final cleaned outputs as protected paths to the cleanup tool.
- Omit the cleanup `--summary` option by default so cleanup does not create a third retained artifact; use it only when the user requested an audit summary before cleaning.
...
- Load workbook formulas as formulas (`data_only=False`) during merge, B站 reply-prefix stripping, and header standardization so formula cells are not silently converted to blank values.
...
python tools\cleanup_intermediate_outputs.py --intermediate "<raw-merged.xlsx>" --intermediate "<raw-merged.summary.json>" --intermediate "<reply-prefix-stripped-merged.xlsx>" --intermediate "<reply-prefix-stripped-merged.summary.json>" --intermediate "<confirmed-standardized.xlsx>" --intermediate "<confirmed-standardized.standardized.summary.json>" --intermediate "<confirmed-cleaned.deletions.csv>" --intermediate "<confirmed-cleaned.summary.json>" --protect "<original-input1.xlsx-or-csv>" --protect "<confirmed-cleaned.xlsx>" --protect "<confirmed-cleaned.csv>"
```
...
- Formula cells remain formulas through merge, B站 reply-prefix stripping, and header standardization.
