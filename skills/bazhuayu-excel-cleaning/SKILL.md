...
- Standardize platform-specific headers and move their complete columns into the fixed output schema.
...
The executable configuration is in `config/comment-cleaner.json` and `config/header-standardizer.json`.

## Execution Steps

1. Accept only the file paths explicitly provided by the user. Never scan a folder for additional inputs.
2. Determine the product name and data source by the fixed rules in `references/naming-and-retention.md`, show all planned output filenames, and obtain the required confirmation.
...
5. Run `scripts/standardize_excel_headers.py`, return the standardized workbook, and wait for explicit approval before cleaning.
