...
class WorkflowDocsTest(unittest.TestCase):
    def test_skill_records_validated_multi_file_workflow(self) -> None:
        ...
        self.assertIn("`评论内容`: `评论内容`, `评论`, `content`", skill)
        self.assertIn("`产品名`: `产品名`, `购买产品`, `商品名称`, `商品`, `评论日期与产品`", skill)
        self.assertIn("`点赞数`: `点赞数`, `点赞量`", skill)
        self.assertIn("`点赞数`: `点赞数`, `点赞量`, `Digg Count`, `like_count`", skill)
        self.assertIn(
        ...
        self.assertIn(
            "When the source header is `timestamp`, `createTime`, `create_time`, `createdAt`, `created_at`, `publishedAt`, `published_at`, `publishedTime`, `published_time`, `date`, `Date`, `time`, `Time`, or another configured platform time alias",
            skill,
        )
        self.assertIn("Keep only year, month, and day; do not output hours, minutes, or seconds.", skill)
        self.assertIn("For Chinese `评论时间` or `评论日期` source columns, convert only numeric timestamps or date-time text that includes a time component to `YYYY-MM-DD`", skill)
        self.assertIn("TikTok aliases such as `createTime`, `评论`, `text`, `Digg Count`, `diggCount`, `回复数`, and `replyCommentTotal`", skill)
        ...
        self.assertIn("Chinese comments whose trimmed length is less than or equal to 7 characters are deleted.", skill)
        self.assertIn("Chinese comments delete by character length; non-Chinese comments delete by deterministic word count.", skill)
        self.assertIn("Non-Chinese comments with four or fewer words are deleted.", skill)
        self.assertIn("For unspaced non-Chinese scripts, only very short text with four or fewer characters is deleted by the short-text rule.", skill)
        self.assertIn("Pure numeric comments keep the legacy seven-character threshold for backward compatibility.", skill)
        self.assertIn("When adding a fixed delete word later, add confirmed equivalents for Chinese, English, Japanese, Korean, Spanish, Thai, and Hindi where applicable.", skill)
        self.assertIn("The complete fixed delete word lists are stored in `config/comment-cleaner.json`", skill)
    ...
    def test_agents_records_non_destructive_merge_then_standardize_flow(self) -> None:
        ...
        self.assertIn("中文 `评论时间` 或 `评论日期` 源列只在值为数字时间戳或带时分秒的日期时间文本时转换为 `YYYY-MM-DD`", agents)
        self.assertIn("`content` -> `评论内容`", agents)
        self.assertIn("`评论`、`text`", agents)
        self.assertIn("`like_count` -> `点赞数`", agents)
        self.assertIn("`Digg Count`、`likeCount`", agents)
        self.assertIn("`回复数`、`replyCount`", agents)
        ...
        self.assertIn("删除首尾空白后长度小于等于 7 的中文主评论", agents)
        self.assertIn("Chinese comments delete by character length; non-Chinese comments delete by deterministic word count.", agents)
        self.assertIn("Non-Chinese comments with four or fewer words are deleted.", agents)
        self.assertIn("For unspaced non-Chinese scripts, only very short text with four or fewer characters is deleted by the short-text rule.", agents)
        self.assertIn("Pure numeric comments keep the legacy seven-character threshold for backward compatibility.", agents)
        self.assertIn("When adding a fixed delete word later, add confirmed equivalents for Chinese, English, Japanese, Korean, Spanish, Thai, and Hindi where applicable.", agents)
        self.assertIn("完整固定清理词清单以 `config/comment-cleaner.json` 为准", agents)
        ...
        self.assertIn("TikTok/YouTube 平台表头映射", agents)
