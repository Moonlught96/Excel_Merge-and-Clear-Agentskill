...
若源表头为淘宝 `评论日期与产品`，只能按固定格式拆分：开头的 `YYYY年M月D日`、`YYYY/M/D` 或 `YYYY-M-D` 进入 `评论日期`，日期后可选的 `已购：` 之后的文本进入 `产品名`。若源表已有 `购买产品`、`产品名`、`商品名称` 或 `商品` 列，则直接映射到 `产品名`。若独立产品列与 `评论日期与产品` 同时存在，独立产品列优先；独立列中空白的单元格只能使用本轮命名确认中用户明确给出的产品名，通过 `tools/standardize_excel_headers.py --product-name` 原样填入，不得回退读取同一行的组合字段。若该来源产品字段不存在或该单元格为空，则同样只能使用本轮命名确认中用户明确给出的产品名；源字段非空时不得覆盖，且不得根据评论内容、文件内容或 AI 推断产品名。`子评论数/追评数` 是标准输出必保留列，但源文件没有对应表头时允许保留空列。不得使用 AI 或语义判断拆分产品名。
...
不得因用户只说“清洗这个文件”而跳过命名确认、单/多文件确认、合并确认、标准化审查与批准、技术名词闸门或适用的 X 推文保留关键词闸门。只有本轮已完成这些前置阶段并完成技术名词清单确认后，才能询问：
