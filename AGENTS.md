...
执行防护为固定规则：`tools/clean_excel_comments.py` 的 CLI 只能读取其相对 Skill 或仓库内置的正式 `config/comment-cleaner.json`，禁止创建、复制、传入或恢复外部/临时清洗配置；公共清洗 CLI 必须显式传入 `--target-header 评论内容` 与已确认的非空 `--platform`，不得退回按第 3 列清洗。所有平台均执行正式配置中的 URL、链接类和其它固定清理词；`搭载`、`爽翻`为所有脚本组均生效的追加固定清理词。唯一已确认的固定例外是 `twitter-comments`：仅在标准列`评论内容`中，先由正式脚本删除字面 `https://` 起至下一个空白字符前的网址文本，再执行正式固定清理词和其它通用规则。该例外不触及任何其它列，不适用于 X 推文或其它平台，且 `http://` 和其它链接类固定词仍照常生效。用户确认规则变更时必须更新正式根配置、Skill 配置、文档和测试。每轮用户提供的技术名词只能作为 `--technical-term` 运行参数使用，必须先完成当前对话的清单完整性确认；命中 4 个及以上不同技术名词才删除整行，同一术语重复出现只计一次。它们不是固定清理词，不得写入或借外部配置修改正式清洗规则。所有会写入文件的 CLI 在目标已存在时必须同时收到 `--overwrite` 和针对每个确切既有目标的 `--confirm-overwrite <路径>`，仅 `--overwrite` 不得覆盖。默认留存清理已经删除最终清洗文件的 `.deletions.csv` 或 `.summary.json` 后，任何脚本入口均不得在同一最终输出路径重新生成、复制恢复或借 `cleanup_intermediate_outputs.py --summary` 重建该日志；需要重新处理时只能使用新的、经用户确认的输出路径。
...
每次标准化完成后、返回用户确认之前，必须运行 `tools/audit_standardized_comments.py`，并传入标准化输入源文件。审查只做确定性、非语义核验：每个工作表是否具有锁定的完整表头及顺序、是否存在重复或原始身份表头、每个非空 `哈希ID` 是否为 64 位小写十六进制、每个数据行的`点赞数`是否非空、工作表名称/顺序以及源文件与标准化输出的数据行数是否一致。标准化脚本必须在审查前把缺失或空白点赞数写为数值`0`；审查只验证，不改写表格。审查报告只能包含路径、工作表名、表头、计数和问题代码，不得包含评论文本或原始身份值。任一核验失败时，必须停止，不得把标准化表格送入用户确认、技术名词、KOL 清理词或清洗流程；审查通过后仍必须保留原有“标准化后的表格已生成，请确认是否可以进入清洗流程”的用户确认门槛。该审查也必须完全由工具完成，不接入 AI。
...
当用户要求“合并后再清洗”时，对同表头批次必须先合并原始输入文件，合并完成后暂停并确认“是否已经提供并合并完所有需要合并的表格”。只有已确认平台注册的混合精确变体才可在原始合并安全停止后走平台预处理后合并例外。用户确认所有需要合并的表格都已合并后，才允许对原始合并总表运行适用的确定性平台预处理和表头标准化，或对平台预处理合并总表直接表头标准化；标准化完成后必须先通过自动审查，才把标准化后的表格返回给用户确认；用户明确确认标准化表格可以进入清洗流程后，若为 `twitter` X 推文分流必须先完成 X 推文保留关键词提供、完整性确认和确定性筛选，再进入技术名词确认和 KOL 清理词确认；若为 `twitter-comments` X 评论分流则直接进入技术名词确认和 KOL 清理词确认。

合并必须输出到一个新建的独立 Excel 文档/工作簿文件作为总表，不是在原 Excel 文档里新建子表/Sheet，也不能把合并结果写回任何原始输入表格。合并完成后必须先把新合并表返回给用户，再等待用户确认是否进行标准化。

标准化后的表格返回给用户确认后，必须先询问技术名词，再询问 KOL 清理词。技术名词为可选本轮规则：询问“是否有技术名词需要清洗？没有就回复‘没有’；有的话请一次性提供所有技术名词。”；用户给出一个或多个技术名词后，不能立刻进入 KOL 清理词流程，必须先确认“是否已经提供完成所有技术名词？你确认后我再询问 KOL 清理词。”。用户确认后，每个技术名词作为一个独立的 `--technical-term` 参数传入；仅当主评论命中 4 个及以上不同的、用户确认的技术名词时删除整行，同一术语重复出现只计一次。拉丁技术名词按大小写不敏感的完整术语匹配，中文、日文、韩文、泰文、印地文、纯数字和混合文字术语按原样字面包含匹配；不得翻译、扩词、模糊匹配、推断或接入 AI。
...
5. 标准化自动审查通过后，标准化后的表格返回给用户确认；若为 `twitter` X 推文分流，必须先询问“请提供本轮 X 推文保留关键词。仅保留‘评论内容’包含任一关键词的整行数据；请一次性提供所有关键词。”，收到一个或多个关键词后再询问“是否已经提供完成所有 X 推文保留关键词？你确认后我将执行关键词筛选，再进入通用技术名词、KOL 清理词与清洗流程。”，确认后用 `tools/filter_comments_by_keywords.py` 执行筛选；`twitter-comments` X 评论分流及其他平台则直接进入技术名词流程。
   - 标准化后的表格返回给用户确认后，必须先询问技术名词，再询问 KOL 清理词。
   - 技术名词如果没有，让用户回复“没有”；如果有，让用户一次性发来所有技术名词。
6. 如果用户提供一个或多个技术名词，必须再确认“是否已经提供完成所有技术名词？你确认后我再询问 KOL 清理词。”；用户确认后每个技术名词传入一个独立的 `--technical-term` 参数，再进入 KOL 清理词流程。
7. 如果用户提供一个或多个 KOL 清理词，必须再确认“是否已经提供完成所有 KOL 清理词”；用户确认后才允许清洗。
8. 用户确认清理词后，直接运行本项目已有工具：
   ...
   - `tools/filter_comments_by_keywords.py`（仅 `twitter` X 推文分流在标准化审查及用户确认后、技术名词和 KOL 清理词前使用；`twitter-comments` X 评论分流绝不使用；每个确认的保留词传入一个独立 `--keep-keyword` 参数）
   - `tools/clean_excel_comments.py`
   - `tools/cleanup_intermediate_outputs.py`（清洗成功生成 `.xlsx` 和 `.csv` 后立即清理中间产物）
   - 不要让用户手动输入命令。
   - 不要要求用户打开命令行。
9. 如果用户说没有技术名词，运行工具时不要传入任何 `--technical-term` 参数；如果提供技术名词，每个技术名词都作为一个独立的 `--technical-term` 参数传入。
10. 如果用户说没有清理词，运行工具时不要传入任何 `--clean-word` 参数；如果提供清理词，每个清理词都作为一个独立的 `--clean-word` 参数传入。
11. 清洗完成并自动清理本轮中间产物后，只返回清洗后的 `.xlsx` 和 `.csv` 文件链接；只有用户在清洗前明确要求保留日志或摘要用于核对时，才保留并返回这些文件。

## 处理边界

- 原始 Excel/CSV 文件不得修改，必须另存为新文件。
- 单文件清洗时，适用的平台预处理、表头标准化与标准化自动审查必须先于清洗执行；多文件流程中，必须先合并原始输入文件；同表头批次再对原始合并总表执行适用的平台预处理、表头标准化与自动审查。已确认平台注册的混合精确变体只可在原始合并安全停止后先做平台预处理后合并，再直接标准化与自动审查。`twitter` X 推文分流的标准化输出经用户确认后必须先进行本轮确认保留词的确定性行筛选，再进入技术名词、KOL 和通用清洗；`twitter-comments` X 评论分流及其他平台标准化后直接按表头“评论内容”进入技术名词、KOL 和通用清洗。
- 表格内容清洗必须是纯工具规则处理，不能接入 AI 判断某条评论是否应该删除。
- AI 只负责调用工具、检查输出文件是否生成、返回结果路径。
- 不要基于语义、情绪、质量、疑似广告等主观判断删除评论。
- 不要添加用户未确认的额外清理词。
- 不要使用旧的八爪鱼 RPA 画布或不存在的 CLI。
- 三道用户确认（进入合并、合并完整、标准化后进入清洗）以及技术名词、KOL/Twitter 词完整性确认必须来自当前对话中的明确用户回复；不得用 CLI 参数、JSON 状态文件、旧日志、历史确认或“尽快处理”等表述替代或伪造确认。

### 命名来源识别优先级

数据来源只能由用户明确提供的输入文件名或由近到远的父级目录中已登记固定关键词识别；一旦某个更近的文件名/目录段唯一命中平台来源，不得再把更外层的工作区、临时目录、Git worktree、仓库目录或项目目录中的关键词累加为第二来源候选。亚马逊通用关键词仅可被同一输入路径中已登记的亚马逊地区关键词覆盖，以确定 `amazon-japan` 或 `amazon-us`；不得用此例外覆盖其它平台。上述优先级只按路径段、固定关键词和固定地区表处理，不使用 AI、语义或模糊匹配。

### Reddit 分流补充规则

Reddit 只能走独立的 `reddit` 平台预处理分流器，且完整有序原始表头必须精确等于 `记录类型`、`标题`、`作者`、`时间`、`内容`、`点赞数`、`评论/回复数`、`层级`、`是否回复`、`评论ID`、`父ID`。`Reddit` 和 `reddit` 路径/文件关键词固定显示为 `Reddit评论数据` 并计划 `reddit` 分流，但显示命名不等于通过分流校验；任一表头缺失、额外、重复、重排或字面不同都必须停止，不得用其它平台规则、内容、文件名、语言、AI 或语义猜测兜底。固定映射为：`时间` -> 临时`评论日期`，`标题` + `内容`使用固定`\n\n`拼接 -> 临时`评论内容`，`点赞数` -> 临时`点赞数`，`评论/回复数` -> 临时`子评论数/追评数`，`作者` -> 临时`Reddit作者`。`记录类型`、`层级`、`是否回复`、`评论ID`、`父ID`及其它原始列不得进入临时输出、标准化输出或清洗输出；每条主帖、评论和各层回复都必须原顺序作为独立行保留，绝不得重建父子关系、复制父评论、将回复移动到`一级评论`/`二级评论`/`三级评论`或因层级超过三级删除数据。Reddit 没有已确认的稳定账号 ID；仅在整张工作表中以临时`Reddit作者`作为显示名弱伪名化兜底，使用独立`reddit`哈希命名空间，相同研究项目与相同作者得到相同 64 位哈希；原始作者不得进入输出、日志或摘要。Reddit 不执行 Twitter/X 保留关键词筛选，标准化审查和用户确认后直接进入既有技术名词、KOL 清理词与通用清洗流程。形如`reddit-<subreddit>-<post-id>-json-primary`或`reddit-<subreddit>-<post-id>-json-fallback`的导出文件名只包含社区/帖子元数据，绝不得作为产品名；无法从其它允许的确定性来源唯一得到产品名时必须询问用户。上述预处理、标准化、哈希、审查和清洗全部只能由确定性工具完成，不接入 AI 判断。
...
本规则适用于所有平台的通用标准化阶段：`电商平台评分`中的数值、精确 `N out of 5 stars` 和精确 `N 颗星，最多 5 颗星` 都必须写为数值 `N`；文本评分仅在 `N` 位于 1–5 时转换，整数写为整数、小数评分保留为数值小数。`点赞数`中的数值、纯数字、精确 `One person found this helpful`、精确 `N person found this helpful`、精确 `N people found this helpful` 和精确 `N 个人发现此评论有用` 都必须写为整数 `N`；缺失或空白点赞数固定写为数值 `0`。其它非空格式保持原值，绝不翻译、猜测、四舍五入、语义判断或接入 AI。

亚马逊日本和亚马逊美国是两个独立平台分流与标准化规则：`amazon-japan` 只接受完整有序 13 列原始表头 `标题`、`标题链接`、`图片`、`aprofile_链接`、`名称`、`aiconalt`、`查看`、`状态`、`查看1`、`asizebase`、`crhelpfultext`、`asizebase_链接`、`asizebase2`，并按既有固定规则从 `查看` 解析评论日期；`amazon-us` 只接受完整有序 10 列原始表头 `标题`、`标题链接`、`图片`、`aprofile_链接`、`名称`、`aiconalt`、`查看`、`状态`、`查看1`、`asizebase`。`amazon-us` 没有已确认的评论日期源字段，预处理必须通过固定 `empty` 操作输出空的 `评论日期`；不得读取、推断或解析 `查看` 为日期。两个平台都固定将 `标题` 与 `查看1` 以 `\n\n` 合并为评论内容，`aiconalt` 解析为电商平台评分，`asizebase` 解析为点赞数，`名称` 仅作为临时显示名输入以生成哈希 ID；但使用独立平台哈希命名空间，因此同一显示名在日本和美国输出不同哈希 ID。
