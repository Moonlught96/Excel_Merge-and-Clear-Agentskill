...
当用户提供八爪鱼导出的 Excel/CSV 文件进行合并或清洗时，必须执行表头标准化：新建一个独立的标准化 Excel 文档，只保留并排序为 `评论日期`、`评论内容`、`产品名`、`哈希ID`、`点赞数`、`子评论数/追评数`、`一级评论`、`二级评论`、`三级评论`。昵称、IP 数据以及其它不在保留清单内的列不得进入后续文件。该过程必须使用确定性工具，不接入 AI 判断。源表头名称不一致时，只能通过固定别名映射处理；缺少必需标准列的已登记别名时必须停止并让用户确认映射。未登记且不在保留清单内的额外列按删除规则省略，不得猜测为标准列。
...
标准化输出中的 `哈希ID` 是确定性伪名化字段，不是法律意义上的匿名化。只能对 `config/hash-id.json` 中按平台登记的真实用户账号 ID 使用项目级 HMAC-SHA256 转换；同一项目、同一平台、相同原始用户 ID 必须得到相同的 64 位小写十六进制哈希，不同项目或不同平台必须不同。当前只确认 YouTube 的 `author_channel_id`、`authorChannelId`、`Author Channel ID` 和小红书的 `用户ID`；评论 ID、父评论 ID、昵称、用户名、主页链接和来源自带的 `哈希ID` 不得作为输入。缺少已登记真实用户 ID 时 `哈希ID` 留空。项目密钥必须由 Windows DPAPI 保护并保存在用户本地项目密钥仓库，不得进入仓库、输出文件、日志或摘要。整个转换必须由确定性工具完成，不接入 AI。
